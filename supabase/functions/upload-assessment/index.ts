...
serve(async (req) => {
  ...
  try {
    ...
    const recordToInsert = {
      ...
      sources: sources || [],
      detailed_assessment: detailed_assessment || toolData, // Store the full object
    };
    
    const { data, error } = await supabaseAdmin
      .from('ai_tools')
      .upsert(recordToInsert, { onConflict: 'name' });
    ...
    return new Response(JSON.stringify({ success: true, message: `Tool '${name}' imported successfully.` }), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      status: 200,
    })
  } catch (error) {
