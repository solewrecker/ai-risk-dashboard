...
-- Convert text columns to JSONB for better complex data storage
ALTER TABLE public.assessments
  ALTER COLUMN detailed_assessment TYPE JSONB USING 
    CASE 
      WHEN detailed_assessment IS NULL OR detailed_assessment = '' THEN '{}'::JSONB
      WHEN detailed_assessment::text LIKE '{%}' THEN detailed_assessment::JSONB
      ELSE ('"' || detailed_assessment || '"')::JSONB 
    END,
  ALTER COLUMN recommendations TYPE JSONB USING 
    CASE 
      WHEN recommendations IS NULL OR recommendations = '' THEN '[]'::JSONB
      WHEN recommendations::text LIKE '[%]' THEN recommendations::JSONB
      ELSE ('"' || recommendations || '"')::JSONB 
    END,
  ALTER COLUMN azure_permissions TYPE JSONB USING 
    CASE 
      WHEN azure_permissions IS NULL OR azure_permissions = '' THEN '{}'::JSONB
      WHEN azure_permissions::text LIKE '{%}' THEN azure_permissions::JSONB
      ELSE ('"' || azure_permissions || '"')::JSONB 
    END,
  ALTER COLUMN compliance_certifications TYPE JSONB USING 
    CASE 
      WHEN compliance_certifications IS NULL OR compliance_certifications = '' THEN '{}'::JSONB
      WHEN compliance_certifications::text LIKE '{%}' THEN compliance_certifications::JSONB
      ELSE ('"' || compliance_certifications || '"')::JSONB 
    END;

-- Do the same for ai_tools table
ALTER TABLE public.ai_tools
  ALTER COLUMN detailed_assessment TYPE JSONB USING 
    CASE 
      WHEN detailed_assessment IS NULL OR detailed_assessment = '' THEN '{}'::JSONB
